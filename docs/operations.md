...
```bash
gh workflow run punch.yml -f action=clock-in -f dry_run=true
```

Dry-run the evening flow:

```bash
gh workflow run punch.yml -f action=clock-out -f dry_run=true
```

Run the real morning flow:

```bash
gh workflow run punch.yml -f action=clock-in -f dry_run=false
```

Run the real evening flow:

```bash
gh workflow run punch.yml -f action=clock-out -f dry_run=false
```
